...
/* librados types for OMAP operations */
 ...
 */
typedef struct rgw_usage_map_entry {
    char* category;            /**< 类别名称 */
    rgw_usage_data_t data; /**< 使用数据 */
} rgw_usage_map_entry_t;
...
typedef struct rgw_usage_log_entry {
    ...
    rgw_usage_map_t usage_map;      /**< 类别使用量映射 */
...
typedef struct rgw_usage_entries {
    struct {
        ...
        rgw_usage_log_entry_t entry;   /**< 使用条目 */
    }* entries;
    size_t count;
    size_t capacity;
} rgw_usage_entries_t;
 ...
 * @param source 源条目
 */
void rgw_usage_log_entry_aggregate(rgw_usage_log_entry_t* target, const rgw_usage_log_entry_t* source);
 ...
 * @brief Usage 配置
 */
typedef struct rgw_usage_config {
...
/**
 * @brief 读取 Usage 日志 (从 RADOS OMAP)
 *
